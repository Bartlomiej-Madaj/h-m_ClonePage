...
function Products() {
  ...
  return (
    <main className="products">
      {!searched[0] && (
        <section className="products__sidebar">
          {list.map((item, index) => (
      ...
      <section className="products__content">
        ...
        >
          {!searched[0]
            ? handler.title
            : searched[0]?.message || "Twój wynik szukania"}
        </h2>

        {!searched[0] && (
          <span className="products__content__description">
            Od bazy na co dzień po wyraziste kreacje - tutaj znajdziesz
            najmodniejsze trendy i nowe ubrania damskie.
          </span>
        )}
        {!searched[0] && (
          <section className="products__content__list">
